...
interface image { f: File | null, isDownloaded: boolean }
...
export const CreateReviewForm = (props: ICreateReviewCard) => {
  ...
  // fix me { f: null, isDownloaded: false }
  ...
  const submitForm = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (props.authorId !== undefined &&
      ...
      formData.append(
        ...
        JSON.stringify({
          sub: props.authorId ?? '',
          author_rating: parseInt(authorRate),
          name_work: nameWork,
          tags,
          text,
          title,
  ...
  return (
    ...
    >
      ...
      <Form.Group className="mb-3">
        <Form.Label>
          {`${props.labels.labelImage}
          ${image.isDownloaded ? `✔️ ${image.f?.name ?? 0}` : ''} `}
        </Form.Label>
      </Form.Group>
